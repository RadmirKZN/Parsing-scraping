...
for i in range(0, 279, 24):
    ...
    for item in cards:
        ...
        fests_urls_lists.append(fest_url)

#collect fest info
count = 0
fest_list_result = []
for url in fests_urls_lists:
    count += 1
    print(count)
    print(url)
    ...
    try:
        ...
        fes_location_url = "https://www.skiddle.com" + fest_info_block.find("a", class_="tc-white").get("href")

        #get contact details and info
        req = requests.get(url=fes_location_url, headers=headers)
        soup = BeautifulSoup(req.text, "lxml")
